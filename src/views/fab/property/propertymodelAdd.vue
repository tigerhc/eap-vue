<template>
  <div class="addDevice">
    ...
    <el-form ref="addForm" :inline="true" :rules="rules" :model="addList" class="addForm" label-width="150px">
      ...
      <el-form-item label="设备大类" prop="classCode">
        <el-input v-model="addList.parent_type" />
      </el-form-item>
      <el-form-item label="设备小类" prop="classCode">
        <el-input v-model="addList.type" />
      </el-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        parent_type: [{ required: true, message: '设备大类必填', trigger: 'blur' }],
        type: [{ required: true, message: '设备小类必填', trigger: 'blur' }]
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    cancel() {
      ...
      this.$store.dispatch('delView', this.viewObj).then(({ visitedViews }) => {
        this.$router.push({ name: 'views/fab/property/fabAssetModel' })
      })
    }
  }
}
